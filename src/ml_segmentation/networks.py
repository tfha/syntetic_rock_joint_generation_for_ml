...
class ASPP(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv2 = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=6, dilation=6, padding_mode='zeros')
        self.conv3 = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=12, dilation=12, padding_mode='zeros')
        self.conv4 = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=18, dilation=18, padding_mode='zeros')
    ...
    def forward(self, x):
        ...
        return F.relu(self.out_conv(torch.cat([conv1, conv2, conv3, conv4, pool], dim=1)))

class FracSegNetVGG19(nn.Module):
    def __init__(self, num_classes=1):
        super(FracSegNetVGG19, self).__init__()
        # VGG19 Encoder
