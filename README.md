...
There are previous research studies on this topic using supervised machine learning. The major challenge is that the labelling process of rock joint traces is a subjective and labarious. Besides, the ground truth of these dataset cannot be 100% correct. 
Addressing the issue of impossible to achieve 100% correct ground truth of previous studies, we novelly evaluate the performance of models trained on synthetic data and tested on real-world data. The syntetic dataset is generated in a process described in the following paper:
...
**Syntethic rock slope dataset**: A dataset of real-world images of rock joints from rock cuttings in Larvik and RV-4 project in Norway. The rock joints are mapped manually but the labels for the dataset is produced automatically using a process developed in Grasshopper and Rhino. The Rhino/Grasshopper process is described in the paper. Due to the manually rock joint mapping process and artifacts in rendering of the 3D models, the labels are not perfect. This is the key dataset to demonstrate the syntetic dataset generation technique.
...
**Real-world box dataset**: A real world dataset with manually labelled gaps between boxes, imitating rock joint traces but in a controlled setting. The labelled data will be nearly perfect and thus allow an quantitative evaluation.

**Real-world rock slope dataset**: A real world dataset with manually labelled rock joint traces. This dataset is used to evaluate the performance of the models on real-world data. NB! We evaluate the confidence of the groud truth based on the labelling process and the degree of field verification. Dataset with ground truth of high confidence will be evaluated quantitatively, whereas those of low confidence will only be evaluated qualitatively (visually) 

Variations for training and evaluation on the synthetic dataset:

1. <ins>"Transfer learning":</ins> Train and evaluate only on the `synthetic rock slope dataset` combined with `synthetic FracMan dataset`. The goal is to evaluate the performance of the model on synthetic data only and compare the performance of different models on this dataset. Choose the best model based on the performance on this process. Run hyperparameter optimization to find the best hyperparameters for this model.

2. <ins>"Deployment":</ins> Train on the `synthetic rock slope dataset` combined with `synthetic FracMan dataset`, and evaluate on the `real-world box dataset` which has nearly perfect labels. The main goal of the study is to demonstrate good performance for this setup.

3. <ins>"Batch-size test":</ins> Train and evaluate only on the `synthetic box dataset` using different sizes of the dataset, and test on `real-world box dataset`. The goal is to demonstrate the effect of different number of samples. This is a pilot study to be presented in Eurock 2025.
