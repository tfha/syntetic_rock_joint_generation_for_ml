...
> Chiu, J. K. Y., Hansen, T. F., Paulsen, E. M., Mengshoel, O. J. (2025). Automated rock joint trace mapping using a supervised learning model trained on synthetic data generated by parametric modelling. Preprint on arXiv: TBA
...
- **DeepLabv3+** (https://arxiv.org/abs/1802.02611) - combines the encoder-decoder architecture with atrous convolution to capture multi-scale features and improve the segmentation performance.
- **U-net** (https://arxiv.org/abs/1505.04597) - encoder-decoder architecture with skip connections for precise localization.

**Note**: In this study, we primarily evaluated **U-net** and **DeepLabv3+** architectures. Other relevant architectures for rock joint segmentation that may be explored in future work include:
- UNet++ (https://arxiv.org/abs/1807.10165) - improved variant of U-net with redesigned nested skip pathways (available in codebase)
...
- FracSegNet - a custom network designed for fracture segmentation
- CrackSegDiff - diffusion-based segmentation for crack detection

These additional architectures are not currently implemented in this codebase but represent promising directions for future research.
...
Inspired by the methodology from [Wachter et al. (2025)](https://arxiv.org/abs/2506.24093), we evaluate two training strategies for combining synthetic and real data:

- **SM (SimpleMixed)**: Train on synthetic and real data simultaneously, sampled randomly from both datasets
- **FT (Finetune)**: Two-stage training - (1) pretrain on synthetic data until real validation Dice plateaus, (2) finetune on real data with adjusted learning rate

**Experimental Design**: We test 7 different synthetic-to-real ratios (0%, 10%, 30%, 50%, 70%, 90%, 100% real) across 10 experiment types:

**Box-domain experiments** (5 types):
- Box: combined real-world box data
- Cardboard Box: cardboard texture only
- Pattern Box: stone pattern texture only
- Generalisation Cardboard Box: train on pattern, test on cardboard
- Generalisation Pattern Box: train on cardboard, test on pattern

**Slope-domain experiments** (5 types):
- Slope: combined real-world slope data
- Larvik: Larvik site only
- Rv4: Rv4 site only
- Generalisation Larvik: train on Rv4, test on Larvik
- Generalisation Rv4: train on Larvik, test on Rv4

For details on experiment methodology and results, refer to the accompanying research paper.

**Key Implementation Details**:
- Dataset split: 90% train / 10% validation (test set held out separately)
- **Validation sets are 100% real data** (unchanged across all experiments)
- Training set varies in synthetic/real ratio based on experiment configuration
- Early stopping: monitored using Dice score (joints) on real validation data
- Both U-net and DeepLabv3+ architectures evaluated for each experiment

**Note on 0% Real Data**: The 0% real data configuration (pure synthetic training) is available for both SimpleMixed and Finetune strategies. In practice, FT at 0% is equivalent to SM at 0%, as both involve pretraining solely on synthetic data without any fine-tuning stage.
...
The project supports 10 experiment types × 2 strategies (SimpleMixed and Finetune) × 7 real data proportions (0%, 10%, 30%, 50%, 70%, 90%, 100%).

Example experiment names:
- **SimpleMixed (SM)**: `simplemixed_box_10`, `simplemixed_slope_30`, `simplemixed_larvik_50`
- **Finetune (FT)**: `finetune_box_10`, `finetune_slope_30`, `finetune_rv4_50`

For the complete list of experiment types and configurations, refer to the experimental design section above.
...
**Note**: The FT (Finetune) two-stage training strategy has been fully implemented. Stage 1 pretrains on synthetic data, and Stage 2 finetunes on real data with learning rate adjustments to prevent overfitting. For complete training implementation details, refer to the accompanying research paper.

### Additional Training Strategies (Proposed - Not Yet Validated)

**Note**: The following training strategies were proposed in an earlier study but did not show promising results in initial experiments. They are documented here for potential future investigation:
...
These strategies may be revisited in future work with improved methodologies or under different experimental conditions.
...
In addition to quantitative metrics, model predictions are evaluated using structured qualitative criteria on a 1-5 scale:
- Geological recognisability
- Joint persistence and continuity
- Boundary localisation and thickness
- False positives and noise levels

This qualitative assessment is essential because quantitative metrics alone may not capture geological plausibility when ground truth labels are imperfect or incomplete. For detailed qualitative assessment tools and methodology, see the [Results Analysis](#results-analysis) section.
...
When selecting Azure ML compute resources, consider:
- **Batch size**: Larger batch sizes require more GPU memory
- **Model architecture**: Different architectures have varying parameter counts and memory requirements (U-net vs DeepLabv3+)
- **Image resolution**: Training on higher resolution images increases memory usage
- **Typical starting point**: VMs with 16GB+ GPU memory (e.g., Standard_NC6s_v3 with NVIDIA Tesla V100)

The project has been successfully tested on Standard_NC6s_v3 instances. Adjust compute resources based on your specific configuration and budget constraints.
